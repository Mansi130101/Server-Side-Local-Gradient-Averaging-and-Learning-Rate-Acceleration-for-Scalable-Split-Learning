...
parser.add_argument(
    "--print-freq", "-p", default=1, type=int, help="print frequency (default: 10)"
)
...
parser.add_argument(
    ...
    default="contrast",
    type=str,
    help="ways to estimate the task-relevant info I(x, y)" "[contrast|cross_entropy]",
)
...
parser.add_argument(
    ...
    default=0.0,
    type=float,
)  # \alpha_1 for 1st local module
parser.add_argument(
    "--beta_1",
    default=0.0,
    type=float,
)  # \beta_1 for 1st local module
parser.add_argument(
    "--beta_2",
    default=0.0,
    type=float,
)  # \beta_2 for 1st local module
parser.add_argument(
    "--gamma_1",
    default=0.0,
    type=float,
)  # gamma_1 of contrastive loss
parser.add_argument(
    "--gamma_2",
    default=0.0,
    type=float,
)  # gamma_2 of contrastive loss

parser.add_argument(
    "--skewness_ratio",
    default=0.5,
    type=float,
)  # Skewness between labels
...
parser.add_argument(
    "--batch_size", type=int, default=128, help="batch size of clients."
)
parser.add_argument(
    "--mixing_mode",
    type=str,
    default="pooling",
    help="mixing method used on worker outputs",
)
parser.add_argument(
    "--local_label_size",
    type=int,
    default=25,
    help="No of samples of one label in a client dataset",
)
...
parser.add_argument(
    "--comm_flag", type=float, default=0.0, help="When to stop averaged gradients"
)
...
training_configurations = {
    "resnet": {
        ...
        if args.dataset in ["cifar10", "svhn", "fmnist"]
        else 128,
...
record_path = (
    ...
    + str(args.layers)
    + "_K_"
    + str(args.local_module_num)
    + "_"
    + str(args.name)
    + "/multiworker/week13/worker_"
    + str(args.workers)
    + "/"
    + "_mixing_mode_"
    + str(args.mixing_mode)
    + "_local_label_size_"
    + str(args.local_label_size)
    + "_batchsize_"
    + str(args.batch_size)
    + "_epochs_"
    + str(args.epoch)
...
def main():
    ...
    if args.dataset == "cifar":
        train_data = datasets.CIFAR10(
            root="/content/images", train=True, download=True, transform=transform_train
        )
        test_data = datasets.CIFAR10(
            root="/content/images", train=False, download=True, transform=transform_test
        )
    ...
    train_indices, val_indices = sort_multi_indices(
        train_data, args.workers * args.local_label_size, 10, "train", args.dataset
    )
    test_indices, _ = sort_multi_indices(test_data, 1000, 10, "test", args.dataset)

    # PROPOSED
    train_worker_indices = divide_indices(
        train_indices, args.workers * args.local_label_size, 10, workers=args.workers
    )
    assert_distribution(train_data, train_worker_indices, None)
    # assert_distribution(train_data, None, val_indices)
    train_loaders, val_loader, test_loader = create_dataloader(
        train_data,
        test_data,
        train_worker_indices,
        val_indices,
        test_indices,
    ...
    central_server = eval(
        "networks.resnet_mixup.resnet" + str(args.layers) + "_chunk2"
    )(
        local_module_num=args.local_module_num,
        batch_size=training_configurations[args.model]["batch_size"],
        image_size=image_size,
    ...
    central_optimizer = torch.optim.Adam(central_server.parameters(), lr=server_lr)  # ,
    ...
    for epoch in range(start_epoch, training_configurations[args.model]["epochs"]):
        ...
        save_checkpoint(
            ...
            is_avg_best,
            checkpoint=central_check_point,
        )
    ...
    test_prec = validate(
        val_loader, workers, central_server, args.workers, epoch, "test", start_time
    )
    avg_test = 0
    for num in range(args.workers):
    ...
    for i, batch in enumerate(zip(*train_loaders)):
        ...
        if args.mixing_mode == "concat_batch_pool":
            for num in range(args.workers - 1):
                ...
                targets_onehot[num] = (
                    0.9 * targets_onehot[num] + 0.1 * targets_onehot[num + 1]
                )
            outputs[args.workers - 1] = (
                0.9 * outputs[args.workers - 1] + 0.1 * outputs[0]
            )
            targets_onehot[args.workers - 1] = (
                0.9 * targets_onehot[args.workers - 1] + 0.1 * targets_onehot[0]
            )
        ...
        if args.mixing_mode == "pooling":
            for num in range(args.workers):
                ...
                input_weight = torch.unsqueeze(
                    torch.unsqueeze(torch.unsqueeze(weight, 1), 2), 3
                )
                input_weight = input_weight.expand(
                    -1, outputs[num].size(1), outputs[num].size(2), outputs[num].size(3)
                )
                if torch.cuda.is_available():
        ...
        for num in range(args.workers):
            if args.mixing_mode == "pooling":
                ...
                if epoch < args.comm_flag * args.epoch:
                    ...
                    backprop[num].backward(
                        central_input.grad[num * s : (num + 1) * s, :]
                    )
            elif args.mixing_mode == "concat_batch_pool":
                s = int(central_input.grad.size(0) / args.workers)
                if num == 0:
                    backprop[num].backward(
                        0.9 * central_input.grad[0 * s : (1) * s, :]
                        + 0.1 * central_input.grad[(args.workers - 1) * s :, :]
                    )
                else:
                    backprop[num].backward(
                        0.9 * central_input.grad[num * s : (num + 1) * s, :]
                        + 0.1 * central_input.grad[(num - 1) * s : (num) * s, :]
                    )
        #'''
        for num in range(num_workers):
            optimizers[num].step()
...
def validate(
    val_loader, workers, central_server, num_workers, epoch, mode="val", start_time=0
):
        ...
        with torch.no_grad():
            for num in range(num_workers):
                ...
                if args.mixing_mode == "concat_channels":
                    prec.append(
                        accuracy(
                            logits[:, num * 10 : (num + 1) * 10].data, target, topk=(1,)
                        )[0]
                    )
                else:
