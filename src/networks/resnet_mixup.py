...
from .auxiliary_nets import AuxClassifier, Decoder
...
class InfoProResNet_Chunk2(nn.Module):
    def __init__(
        ...
        # self.inplanes = wide_list[1]    # For larger server
        self.inplanes = wide_list[2]  # For smaller server
        self.dropout_rate = dropout_rate
        self.feature_num = wide_list[-1]
...
class InfoProResNet(nn.Module):
    def __init__(
        ...
        try:
            self.infopro_config = (
                InfoPro_balanced_memory[arch][dataset][local_module_num]
                if balanced_memory
                else InfoPro[arch][local_module_num]
            )
        except:
    ...
    def forward(
        ...
        if self.training:
            ...
            if local_module_i <= self.local_module_num - 2:
                if (
                    self.infopro_config[local_module_i][0] == stage_i
                    ...
                    loss_ixx = eval(
                        "self.decoder_" + str(stage_i) + "_" + str(layer_i)
            ...
            chunk2_loss = self.criterion_ce(logits, target)
            chunk2_loss *= beta_2

            # hidden1 = self.projection_head1(hidden)
            # hidden2 = self.projection_head2(x)

            return logits, chunk1_loss, chunk2_loss, hidden, x
        else:
            x = self.conv1(img)
            x = self.bn1(x)
            x = self.relu(x)
