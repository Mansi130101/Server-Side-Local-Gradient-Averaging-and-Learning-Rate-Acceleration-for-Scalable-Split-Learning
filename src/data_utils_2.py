...
def sort_multi_indices(data, size=100, nclass=10, split="train", dataset="cifar10"):
    ...
    for i in range(nclass):
        used_indices = np.append(
            total_indices[i * label_size : i * label_size + size], used_indices
        )
        unused_indices = np.append(
            total_indices[i * label_size + size : (i + 1) * label_size], unused_indices
        )
    ...
    if len(unused_indices) != 0:
        for i in range(nclass):
            val_indices = np.append(
                val_indices,
                unused_indices[
                    i * (label_size - size) : i * (label_size - size) + 1000
                ],
            )
...
def divide_indices(indices, size, no_of_classes=10, workers=2):
    ...
    for i in range(no_of_classes):
        for j in range(workers):
            ...
            train_indices[j].extend(
                indices[
                    i * size
                    + j * label_per_worker : i * size
                    + (j + 1) * label_per_worker
                ]
            )
...
def create_dataloader(
    train_data, test_data, train_indices, val_indices, test_indices, batch_size
):
    ...
    for i in range(len(train_indices)):
        train_dataloader = torch.utils.data.DataLoader(
            train_data, batch_size=batch_size, shuffle=False, sampler=train_indices[i]
        )
        train_dataloaders.append(train_dataloader)

    val_dataloader = torch.utils.data.DataLoader(
        train_data, batch_size=1024, shuffle=False, sampler=val_indices
    )
    test_dataloader = torch.utils.data.DataLoader(
        test_data, batch_size=1024, shuffle=False, sampler=test_indices
    )
    return train_dataloaders, val_dataloader, test_dataloader


def calc_memory_usage(x):
    ele_size = x.element_size()
    elements = x.nelement()
    total_size = ele_size * elements / (1024**2)
    return total_size
