...
record_path = './' \
              ...
              + ('_num_' + str(args.num) if args.num else '') \
...
def main():
    ...
    if args.model == 'resnet':
        for num in range(args.workers):
            ...
            model1 = eval('networks.resnet_4.resnet' + str(args.layers))\
  ...
  for i,batch in enumerate(zip(*train_loaders)):
    ...
    for num in range(num_workers):
      output1, chunk1_loss1, chunk2_loss1, hidden11, hidden12= workers[num](img=x[num],
                          ...
                          gamma_2 = gamma_2, chunknum=chunknum,epoch=epoch,num=args.num)
