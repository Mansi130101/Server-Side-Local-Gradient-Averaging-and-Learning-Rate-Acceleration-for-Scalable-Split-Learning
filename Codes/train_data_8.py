  ...
  for epoch in range(start_epoch, epochs):#training_configurations[args.model][echunk]):
    for num in range(num_workers):
      ...
      for i,batch in enumerate(zip(*train_loaders)):
        ...
        optimizers[num][0].zero_grad()

        output1, chunk1_loss1, hidden11= workers[num][0](img=x[num],target = target[num])
        client_output = hidden11.clone().detach().requires_grad_(True)

        # ---------- server forward/backward training ------------
        optimizers[num][1].zero_grad()

        output2, chunk2_loss1 = workers[num][1](img=client_output, target = target[num])
        
        if num!=num_workers-1:
          chunk2_loss1.backward(retain_graph = True)
        else:
          chunk2_loss1.backward()

        optimizers[num][1].step()
        client_grad = client_output.grad.clone().detach()

        # ----------- client backward training ------------
        if num!=num_workers-1:
          hidden11.backward(client_grad,retain_graph = True)
        else:
          hidden11.backward(client_grad)

        optimizers[num][0].step()
        ...
        prec.append(accuracy(output2.data, target[num], topk=(1,))[0])
        losses2[num].update(chunk2_loss1.data.item(), hidden11.size(0))
...
def load_wts():
      ...
      for key in w_avg.keys():
          for i in range(1, len(a)):
              w_avg[key] += a[i][key]
          w_avg[key] = torch.div(w_avg[key], float(args.workers))
