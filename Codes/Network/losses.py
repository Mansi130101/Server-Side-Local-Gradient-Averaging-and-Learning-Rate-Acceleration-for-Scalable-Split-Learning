...
class M_CosineLoss(torch.nn.Module):
  ...
  def forward(self, X, Y, t1, t2):
    ...
    Z = (Z - torch.min(Z).detach())/(torch.max(Z).detach() - torch.min(Z).detach())  
    ...
    print("+ve = ", self.positive_weight/(s1.item()*(batch_size**2)), "-ve = ", self.negative_weight*s2.item()/(batch_size**2),"\n")
    loss = (self.positive_weight/s1 + self.negative_weight*s2)/(batch_size**2)
    
    f=open("pos_neg_mcosine_exp.csv", "a+")
    csv_writer = csv.writer(f)
    csv_element = [self.negative_weight*s2.item()/(batch_size**2), 
...
class E_CosineLoss(torch.nn.Module):
  ...
  def forward(self, X, Y, t1, t2):
    ...
    print("+ve loss = ", self.positive_weight/(positive_sum*(batch_size**2)), "\n-ve loss = ", self.negative_weight*negative_sum/(batch_size**2),"\n")
    loss = (self.positive_weight/positive_sum + self.negative_weight*negative_sum)/(batch_size**2)
    return loss
...
class LE_CosineLoss(torch.nn.Module):
  ...
  def forward(self, X, Y, t1, t2):
    ...
    loss = (self.positive_weight*positive_loss + self.negative_weight*negative_loss)/(batch_size**2)
    f=open("pos_neg_lecosine_exp.csv", "a+")
    csv_writer = csv.writer(f)
    csv_element = [self.negative_weight*negative_loss.item()/(batch_size**2), 
...
class DistanceLoss(torch.nn.Module):
  ...
  def forward(self, X, Y, t1, t2):
    assert X.size() == Y.size(), "Both the hidden tensors need to be of the same shape."
    batch_size = X.size(0)
    x = F.normalize(X)
    y = F.normalize(Y)
    pos_mask = torch.zeros((batch_size, batch_size))
    ...
    total_loss = (self.negative_weight*negative_loss + self.positive_weight*positive_loss)/(batch_size**2)
    #print("-ve loss = ", self.negative_weight*negative_loss.item()/(batch_size**2))
    #print("+ve loss = ", self.positive_weight*positive_loss.item()/(batch_size**2))
...
class AMCLoss(torch.nn.Module):
  ...
  def forward(self, x, y, t1, t2):
    ...
    geodesic_distance = torch.acos(torch.clip(torch.matmul(x_norm, y_norm.t()), -1.0+1e-7, 1.0-1e-7))
