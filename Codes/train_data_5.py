...
record_path = './' \
              ...
              + '_' + str(args.name) \
              + 'SplitFed/worker_' + str(args.workers) +'/sfl_version_' \
              + str(args.version) + '/' \
    ...
    for num in range(num_workers):
      if chunknum==1:
        if epoch==0:
          ...
          workers[num][0].load_state_dict(a[0]) #,strict=False)
          print("Loaded best checkpoint from epoch ", c[n]['epoch'])
          #workers[num][0].eval()
