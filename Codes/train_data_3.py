...
record_path = './' \
              ...
              + '_' + str(args.name) \
              + '/multiworker/week8/worker_' + str(args.workers) + '/' \
...
for num in range(args.workers):
    ...
    cp.append(args.checkpoint + '_' + str(num))
    check_point.append(os.path.join(record_path, cp[num]))
...
def main():
    ...
    if args.resume: #folder
        ...
        ckp=[]
        for num in range(args.workers):
          resumed_path.append('{}/{}/checkpoint.pth.tar'.format(record_path, cp[num])) #os.path.join(record_path, cp[num]))
          print(resumed_path)
          assert os.path.isfile(resumed_path[num]), 'Error: no checkpoint directory found!'
          #args.checkpoint[num] = os.path.dirname(resumed_path[num])
          cp[num] = os.path.dirname(resumed_path[num])
          ckp.append(torch.load(resumed_path[num]))
          start_epoch = ckp[num]['epoch']
          workers[num].load_state_dict(ckp[num]['state_dict'])
          optimizer.load_state_dict(ckp[num]['optimizer'])
          val_acc_[num]=ckp[num]['val_acc']
          best_prec_[num] += ckp[num]['best_acc']
          #print('best_prec_',best_prec_[num],num)
          #print(val_acc_[num])
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        # train for one epoch
        train(train_loaders, workers, args.workers, optimizers, epoch)
        
        # evaluate on validation set
        prec = validate(val_loader, workers, args.workers, epoch)
        ...
        for num in range(args.workers):
            ...
            if num==args.workers -1:
                ...
                for n in range(args.workers):
                  avg_prec+=best_prec_[n]/args.workers
                best_avg_prec = max(best_avg_prec, avg_prec)      
                print('Best avg accuracy: ', best_avg_prec) 

    test_prec = validate(test_loader, workers, args.workers, training_configurations[args.model]["epochs"], "test", start_time)
...
def train(train_loaders, workers, num_workers, optimizers, epoch):
          ...
          if args.gamma_1 != 0:
            for num in range(num_workers):
              ...
              for n in nlist:
                  ...
                  l2=loss[num].item()
                  if num==0:
                    print('Contrastive Loss Added = ', l2-l1,'Other Losses = ', l1,'num',num,'n',n,'c1')
          ...
          if args.gamma_2 != 0:
            for num in range(num_workers):
              ...
              for n in nlist:
                  ...
                  l1=loss[num].item()
                  loss[num] += (c2l[num][n])/(len(nlist))
                  l2=loss[num].item()
                  #print('Contrastive Loss Added = ', l2-l1,'Other Losses = ', l1,'num',num,'n',n,'c2')
...
def validate(val_loader, workers, num_workers, epoch, mode="val",start_time=0):
